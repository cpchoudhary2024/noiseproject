...
def test_wind_limit_boundary_at_microphone_height():
    """Default rule: the NSW limit applies to the mean at microphone height.

    Mic factor 0.641939 at 1.5 m: 15 kt -> 4.95 m/s (keep), 16 kt -> 5.28 m/s (exclude).
    """
    ...
    assert _reasons(b)[4] == "clean"
    assert _reasons(b)[5] == "wind"


def test_station_wind_option_excludes_without_height_reduction():
    """The stricter option screens the station's own wind, so both blocks go."""
    k15 = {f"01:{m:02d}": 15.0 for m in range(0, 15)}
    k16 = {f"01:{m:02d}": 16.0 for m in range(15, 30)}
    cfg = ScreenConfig(screen_station_wind=True).validate()
    b = _classify(_onemin("2026-07-01 00:00", "2026-07-01 03:00", wind={**k15, **k16}), cfg=cfg)
    assert _reasons(b)[4] == "wind" and _reasons(b)[5] == "wind"
...
def test_gust_and_short_wind_peak_are_measured_and_reported():
    """Station maxima are always recorded; they exclude only under the option.

    A single gust does not move a 15-minute mean much, so under the published
    rule these blocks stay while the maximum is still available for review.
    """
    om = _onemin("2026-07-01 00:00", "2026-07-01 03:00")
    ...
    assert _reasons(b)[3] == "clean"
    strict = _classify(om, cfg=ScreenConfig(screen_station_wind=True).validate())
    assert _reasons(strict)[3] == _reasons(strict)[5] == "wind"
...
@pytest.mark.skip(reason="written against analysis.chart_generator, which the interface redesign removed; figure captions are now covered by the report tests")

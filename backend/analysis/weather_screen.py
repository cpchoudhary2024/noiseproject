...
3. A block is excluded, in priority order, for: precipitation; thunder;
   observed station wind/gust or estimated block-mean microphone wind above the limit; snow cover on its
...
4. Wind is measured at the anemometer (ASOS: 10 m) and converted to microphone
   ...
   This is a model estimate, not an on-site measurement. Station wind/gust
   maxima are also screened without height reduction (a conservative platform rule).
...
def classify_blocks(minutes: pd.DataFrame, start_utc: pd.Timestamp, end_utc: pd.Timestamp,
    ...
    windy = ((out["wind_mic_ms"] > cfg.wind_limit_ms)
             | (out["wind_station_max_ms"] > cfg.wind_limit_ms)).fillna(False).to_numpy()
    conds = [
        out["precip"].to_numpy(),
